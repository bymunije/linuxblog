...
cron进程根据`EDITOR`来确定使用哪个编辑器编辑crontab文件,UNIX和LINUX用户可使用vi，做法是在$HOME目录下的. profile文件中加入下面一行:
...
```
# 每天八点半执行readfree.py
30 8 * * * /usr/software/anaconda3/bin/python3.6 /home/byzhang/Read/readfree.py >> /home/byzhang/Read/log1.txt 2>&1
```
#### 列出crontab文件
...
crontab -l
```
输出`30 8 * * * /usr/software/anaconda3/bin/python3.6 /home/byzhang/Read/readfree.py >> /home/byzhang/Read/log1.txt 2>&1`

而且可以使用下面的方法对crontab文件备份:

```
crontab -l > /home/byzhang/Read/mycron
```
...
```
，需要在crontab命令中使用`usr/software/anaconda3/bin/python3.6 /home/byzhang/Read/readfree.py`，同时在python文件中添加相应命令，包括行首添加的`#!/usr/software/anaconda3/bin/python`和文中添加的
